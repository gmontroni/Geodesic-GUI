...
{
	int      finished;
	size_t   i,startind,endind,whichneighbor,ndone,closest;
	double   closestD,arclength; 
	double   INF,ZERO,olddist;
	HeapNode *Min;
	HeapNode Temp;
...
)
{
	...
	if (nrhs < 2 || nrhs > 4)
		mexErrMsgTxt("2 or 3 input arguments allowed.");
	if (nrhs < 3)
		maxD = mxGetInf();
	else if (mxGetM(prhs[2]) != 1 || mxGetN(prhs[2]) != 1)
		mexErrMsgTxt("Max distance must be a scalar.");
	else
		maxD = *mxGetPr(prhs[2]);
	...
	if (mxIsSparse(prhs[0]) == 1)
	{
		...
		for (i=0; i<MS; i++)
		{
			...
			dodijk_sparse(M, N, S, Dsmall, sr, irs, jcs, A, theHeap, maxD);
